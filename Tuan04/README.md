...
- Góc quay dương sẽ xoay ảnh theo chiều ngược kim đồng hồ
- Ví dụ: `image.rotate(45)` xoay ảnh 45 độ
- Ảnh được xoay quanh tâm của nó, giữ nguyên kích thước
...
- Đọc ảnh xám: `Image.open()` và chuyển sang grayscale bằng `ImageOps.grayscale()`
- Chuyển ảnh xám thành ma trận NumPy: `np.array(im_gray)` để có thể thực hiện phép toán ma trận
- Phân tích SVD: `U, s, V = np.linalg.svd(im_gray, full_matrices=True)` để tách ảnh thành ba ma trận U, S, V
- Tạo ma trận S: Chuyển vector s thành ma trận đường chéo bằng `np.diag(s)`
- Khôi phục ảnh: Nhân lại ba ma trận `A = U × S × V^T` để được ảnh gốc
- Nén ảnh: Chỉ giữ lại n thành phần đầu tiên (lấy n cột đầu của S và n hàng đầu của V), nhân lại để tạo ảnh xấp xỉ
...
- Tạo ma trận biến đổi affine: `np.float32([[1, 0, tx], [0, 1, ty]])`
  - `tx`: số pixel dịch chuyển theo chiều ngang (dương = sang phải, âm = sang trái)
  - `ty`: số pixel dịch chuyển theo chiều dọc (dương = xuống dưới, âm = lên trên)
- Dùng `cv2.warpAffine()` để áp dụng phép dịch chuyển:
  - Tham số: ảnh gốc, ma trận biến đổi M, kích thước đầu ra
- Điều chỉnh kích thước đầu ra `(cols + tx, rows + ty)` để không bị cắt mất phần ảnh khi dịch chuyển
- Ví dụ: dịch 100 pixel sang phải với `tx=100, ty=0`

**Rotation - Xoay ảnh:**
- Tạo ma trận quay: `cv2.getRotationMatrix2D(center, angle, scale)`
  - `center`: tâm quay (thường là trung tâm ảnh `(cols//2, rows//2)`)
  - `angle`: góc quay tính bằng độ (dương = ngược chiều kim đồng hồ)
  - `scale`: tỷ lệ phóng đại (thường là 1 để giữ nguyên kích thước)
- Áp dụng ma trận quay bằng `cv2.warpAffine()` với kích thước đầu ra
- Ví dụ: xoay 45 độ quanh trung tâm ảnh
...
**Matrix Operations - Phép toán ma trận:**
- Đọc ảnh xám trực tiếp: `cv2.imread('file.png', cv2.IMREAD_GRAYSCALE)` không cần chuyển đổi
- Phân tích SVD: `U, s, V = np.linalg.svd(im_gray, full_matrices=True)` để tách ảnh thành ba ma trận U, S, V
- Tạo ma trận S: Tạo ma trận toàn số không với cùng kích thước ảnh, đặt các giá trị đường chéo từ vector s
- Khôi phục ảnh: Nhân lại ba ma trận `A = U × S × V^T` để được ảnh gốc
- Nén ảnh: Chỉ giữ lại n thành phần đầu tiên (lấy n cột đầu của S và n hàng đầu của V), nhân lại để tạo ảnh xấp xỉ với chất lượng giảm nhưng vẫn giữ thông tin quan trọng
...
## Giải thích về SVD 
### SVD là cách tách một ma trận thành ba phần nhỏ hơn. Hiểu đơn giản, nó giống như phân tích một bức ảnh thành các mảnh ghép quan trọng nhất.
**Công thức:** Ảnh = U × S × V
